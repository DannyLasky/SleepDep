...
outputDirBase = "P:\Jones_Maganti_Shared\Sleep Dep\Output 06-07-23";

for fileCount = 1:length(fileArr)
    currentFile = fileArr(fileCount);
    cd('P:\Jones_Maganti_Shared\Sleep Dep\EDFs and TXTs\Cohort 3 EDFs and TXTs\');
    ...
    %% Change scored NREM during sleep deprivation to wake
    %{
    homeoScoresBad = homeoScores;
    
    if ~contains(fileNameEDF,["baseline","Baseline","recovery","Recovery"])
        for n = 1351:4050
            if homeoScores(n) == 2
                homeoScores(n) = 1;
            end
...
%% Determine NREM cluster threshhold by fitting to the shorter bordering NREM length
    figure
    histogram(sqrt(shorterNREMLength), logspace(0, 2, 41))      % Cap of 10000 epoch long bout
    set(gca, 'xsc', 'log')

    [yFitNREM,binEdgesNREM] = histcounts(sqrt(shorterNREMLength), logspace(0, 2, 41));
    xFitNREM = 1:length(yFitNREM);

    NREMLimit = 100;     % Hand select threshhold until curve fitting works

    invalidNREMFilter = shorterNREMLength > NREMLimit;
...
%% Defining clusters and remove ones that are too small to be deemed clusters
    clusterMatrix = [shorterNREMLength, run_starts(NREMIndx), currNREMEnd];
...
%% Display NREM delta power of all NREM, clustered NREM, and clean NREM
    ...
    for n = 1:4
        ...
        if n > 1
            ...
            deltapwr = finalMatrix(:,1);
            deltapwr(ZScoreFilter) = NaN;
        end
